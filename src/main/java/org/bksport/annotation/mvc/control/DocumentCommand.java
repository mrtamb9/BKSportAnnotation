...
public class DocumentCommand extends SimpleCommand {
  ...
  public void execute(INotification notification) {
    DocumentProxy dProxy = (DocumentProxy) facade
        .retrieveProxy(AppFacade.DOC_PROXY);
    if (notification.getName().equals(AppFacade.ADD_DOCS_CMD)) {
      int docCount = 0;
      ...
      for (File file : files) {
        ...
        if (file.isFile()
            && (file.getName().endsWith(".html")
                || file.getName().endsWith(".htm") || file.getName().endsWith(
                ".txt"))) {
        ...
        if (file.isFile() && file.getName().endsWith("json.tsv")) {
      ...
      sendNotification(AppFacade.UPDATE_STATUS_CMD, "Add " + notification.getBody()
          + " documents successfully.");
    }
    if (notification.getName().equals(AppFacade.REMOVE_DOCS_CMD)) {
      dProxy.removeDocument((String) notification.getBody());
      sendNotification(AppFacade.DOC_REMOVED,
          (String) notification.getBody());
      sendNotification(AppFacade.UPDATE_STATUS_CMD,
          "Remove " + notification.getBody() + " document successfully.");
    }
    ...
    if (notification.getName().equals(AppFacade.LOAD_DOCS_CMD)) {
      ...
      sendNotification(AppFacade.UPDATE_STATUS_CMD, "Load " + docs.size()
          + " documents.");
    }
    
    
    
    if (notification.getName().equals(AppFacade.ANNOTATE_DOCS_CMD)) 
    {
  ...
  private class DocumentAnnotator extends
      SwingWorker<ArrayList<Document>, ArrayList<Document>> {
    ...
    protected ArrayList<Document> doInBackground() throws Exception {
      ...
      for (int i = 0; i < docs.size(); i++) {
        AppFacade.getInstance().sendNotification(AppFacade.UPDATE_STATUS_CMD,
            "Annotate " + docs.get(i).getTitle());
        docs.get(i).setKIMDoc(
            connector.annotate(connector.create(docs.get(i).getContent())));
        ...
        AppFacade.getInstance().sendNotification(AppFacade.DOC_ANNOTATED,
            docs.get(i).getKIMDoc().getDocumentId());
      }
      return docs;
    }
