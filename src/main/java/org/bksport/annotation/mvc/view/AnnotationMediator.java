  ...
  public void handleNotification(final INotification notification) {
    ...
    if (name.equals(AppFacade.ANN_LOADED)) {
      if (getViewComponent() == null)
        setViewComponent(new AnnotationDialog((Frame) facade.retrieveMediator(AppFacade.CONTAINER_MED).getViewComponent(), true));
      AnnotationDialog dialog = (AnnotationDialog) getViewComponent();
      ArrayList<KIMAnnotation> annList = (ArrayList<KIMAnnotation>) notification.getBody();
