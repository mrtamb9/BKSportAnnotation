...
public class ContainerMediator extends Mediator {
  ...
  public void handleNotification(final INotification notification) {
    ...
    {
      ((ContainerFrame) (getViewComponent())).updateStatus((String) notification.getBody());
    } 
    else if (name.equals(AppFacade.UPDATE_STATUS_PROGRESS)) 
    {
      ((ContainerFrame) (getViewComponent())).updateStatusProgress((Integer) notification.getBody());
    } 
    else if (name.equals(AppFacade.DOCS_LOADED)) 
    {
      ...
      {
        ((ContainerFrame) (getViewComponent())).viewDocument((KIMDocument) docs.get(i));
      }
    } 
